...
56. V5 train/validation model protocol frozen before training: one shared-slot scaler from440 deduplicated observable train-context `(episode,state)` rows only (368 passive+72 action), no future/val/test and no old scaler. Action and two-outcome passive-branch tracks each compare scratch/Friday/V4 with seeds51001-3, fixed budgets and all120 host permutation augmentation. Action selection uses validation state+0.25 edge to avoid deterministic action-label dominance; passive uses frozen branch objective. Primary threshold.5; oracle coverage/calibration caveats explicit. Tests pass action93/branch91 finite gradient tensors, source loads Friday action82+11missing/V493+0, passive82+9missing. Protocol SHA cc5801a..., scaler SHA3928af5..., source commit659f20a; `test_unlock=false`. Separate post-validation evaluation freeze mandatory; no test inference.
