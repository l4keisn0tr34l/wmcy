...
31. Six-permutation pair audit rejects the tuned identity pair score as robust: tuned top-1 ranges 2/14–8/14; non-identity choices map back to the identity choice only ~1.3%. Published model is also unstable. Implement shared-weight pair scoring; full equivariance still requires a graph encoder. See `docs/PAIR_EQUIVARIANCE_AUDIT.md`.
