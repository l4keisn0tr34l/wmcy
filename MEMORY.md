...
28. Senior independently reported transformer failure on scarce data, stronger RSSM forecasting, weak downstream classes, and improvement when RSSM was unfrozen. Their artifacts/protocol are unavailable, so this is an external hypothesis only. Next project ablation should compare frozen two-stage, unfrozen fine-tuned, joint-from-scratch, and KL-free RSSM on identical V2 splits. Defer DANN and ensembles until domain/complementarity evidence exists.
