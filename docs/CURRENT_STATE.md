...
Same-scaler aligned scratch passive MAE `.468434` is slightly worse than action `.459835`, but passive active-feature MAE `.661994` is better than action `.787490`; cohorts/objectives differ, so this is not a pure conditioning ablation. Friday does not improve scratch dynamics; V4's lower passive point MAE does not change the primary selection. The standalone V5 senior report and11-slide judge deck are now generated from saved metrics only, with the passive warning failure and claim boundaries prominent. See `docs/V5_TEST_RESULTS.md` and the immutable pre-test `docs/V5_FINAL_REVIEW.md`. Historical pending-test statements below are superseded.
...
V5 now contains 80 paired 150-second episodes on a five-host isolated graph: 40 train, 16 validation, and 24 sealed test. It adds balanced quiet/web/admin/mixed background traffic, scan-based and direct-credential permit/block actions, passive stopped/progressing pairs, benign controls, and a test-only intent observability probe. Astra review corrected action/cutoff timing, background duration/source-role shortcuts, acquisition ordering, cohort-profile correlation, cleanup/isolation, and missing freeze enforcement. The 345-feature/20-pair exporter passes 12 synthetic tests and CUDA permutation/finite-gradient plus exact CPU temporal-independence checks. The revised container configuration has not yet undergone full smoke capture; earlier HTTP/SSH/firewall checks were against the draft. The first smoke was manually interrupted and quarantined; two replacements passed before capture freeze. The full V5 corpus is now complete: all 80 planned episodes are raw/derived-complete and independently pass general plus strict V5 validation. Raw durations span 150.000094–150.000615s; every episode has 29 complete five-second states and zero packet drops. Attack-intent versus benign/legitimate mean duration differs by only 15 microseconds, and maximum paired duration difference is 0.456ms, so the historical outcome-dependent censoring shortcut is absent. All actions retain at least 42.294828s after cutoff. Residual limitation: independent absolute grid phase moves paired cutoffs by up to one state (2/20 action families), so pairs are not packet-identical. The first rejected `lab_158` transport attempt is quarantined; its replacement validates. Train/validation-only exports are now built and audited: passive336/168 samples, action24/8, and aligned-passive24/8. Their345-feature schemas match, action/aligned-passive common arrays are exactly equal, whole-family splits remain disjoint, and no test sequence artifact exists. The train/validation model protocol is now implemented and pre-freeze tested but not yet executed: one shared-slot scaler will fit exactly440 deduplicated observable train-context states; scratch/Friday/V4 action and passive-branch candidates use fixed budgets/seeds and validation-only selection. Protocol regression tests pass with finite action/branch gradients, all120 permutation contracts, compatible historical parameter loads, and no old scaler reuse. The first train/validation freeze was invalidated before fit: the inherited V4 CUDA smoke asserted bitwise-zero future perturbation but observed `4.47e-8`, consistent with the already documented `2.98e-8` CUDA tolerance. No optimizer step, checkpoint, validation result, test export, or test inference occurred. The original freeze/scaler/log are archived. Protocol v1.1 runs exact causality and all120 equivariance checks on CPU while retaining CUDA optimization and a `1e-6` CUDA diagnostic tolerance; no scientific hyperparameter or data rule changed. V1.1 then completed three scratch candidates in temporary storage but stopped before Friday optimization because an inherited helper hardcoded `<2e-6` and rejected Friday initialization at `2.62e-6`, despite the declared `<1e-5` protocol. No report/checkpoint was installed and temporary states will not be reused; no test access occurred. V1.2 replaces stale helper gating with protocol-owned CPU causality0, all120 equivariance `<1e-5`, and finite-gradient checks. V1.2 prefreeze tests pass actual scratch/Friday/V4 initialization paths. It is frozen from commit `f88a43b`: protocol SHA `c25e68fc841e86e9949b80b616fbceeb16be3c8781e66044c3bb4c5114b75457`, deterministic scaler SHA `3928af5e911006b713374c826e59162e68c7762d9d95ace15fa2b437ddf44be6`, test locked. All candidates restarted from zero and train/validation is now complete. Scratch is primary for action (selection1.019 vs V41.093, Friday1.214) and passive branching (.932 vs V4.945, Friday.957). Action scratch validation state MAE is.473 vs persistence.566; edge AP.350; LM/pair AP and F1@.5 are1 but deterministic action makes semantics easy; factual state beats opposite action7/8. Passive scratch expected/oracle MAE.461/.460 vs persistence.534, but LM AP.161, F1@.5 zero, pair AP.009, and oracle gain only.0012. All retained models pass independent hash/recompute/causality/equivariance checks; test remains absent. The sealed evaluator now reproduces validation metrics and includes same-scaler state groups, persistence, edge/ATT&CK/LM/pair metrics, factual-vs-opposite actions, expected/oracle passive branches, scenario diagnostics, and a metadata-cutoff test-only intent probe. Atomic test export/evaluation and fail-preservation are implemented. Evaluation freeze has not been created: final Astra-style review is the current gate. See `docs/V5_VALIDATION_RESULTS.md` and `docs/V5_EVALUATION_PROTOCOL.md`.
...
The standalone senior report is now V5-led and accompanied by an11-slide standalone judge deck. Both are generated by `scripts/61_build_v5_judge_materials.py` from the immutable sealed report/provenance and corpus audit; no checkpoint or prediction code is loaded. Report SHA-256: `919a42436b7f0ebf2e629b52e4046810e48d43a99870ca23d7e53c23e9186442`; deck SHA-256: `738fa5aeda761a3afcdf6de104908ba75686bab92dc8c1303e85a730f4ba70d2`.
