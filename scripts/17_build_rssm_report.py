...
<p class="callout bad"><strong>Failed robustness gate:</strong> the apparent pair gain is fixed-slot sensitive and is rejected as a headline. Permutation augmentation did not enforce equivariance. The next decoder must score each source-target candidate with shared weights; full graph equivariance ultimately requires replacing the flattened encoder.</p>
