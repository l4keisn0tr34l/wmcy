...
def train_graph(module: Any, initial_state: dict[str, torch.Tensor], seed: int,
                ...
                selection_mode: str) -> tuple[dict[str, torch.Tensor], dict[str, Any]]:
    module.seed_everything(seed)
    model = GraphRSSM(**config); model.load_state_dict(initial_state)
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    best = deepcopy(model.state_dict()); best_value = math.inf; best_epoch = 0; stale = 0
    ...
    for epoch in range(1, epochs + 1):
        ...
        validation = module.validate_objective(
            ...
            torch.device("cpu"), selection_mode=selection_mode,
        )
        ...
        if validation["selection"] < best_value - 1e-5:
            ...
            best = deepcopy(model.state_dict()); stale = 0
        else: stale += 1
...
def main() -> int:
    ...
    module.seed_everything(7); initial_model = GraphRSSM(**config)
    ...
    pretrained_state, pretraining = train_graph(
        ...
        args.pretrain_epochs, args.pretrain_patience, args.pretrain_learning_rate, "dynamics",
    )
    ...
    torch.save({"model_state_dict": pretrained_state, "model_config": config,
    ...
    for seed in [int(value) for value in args.seeds.split(",") if value.strip()]:
        state, summary = train_graph(
            ...
            args.finetune_epochs, args.finetune_patience, args.finetune_learning_rate, "joint",
        )
    ...
    dynamics, mc = ablation.dynamics_metrics(
        ...
        args.lab_batch_size, selected_seed + 900_000,
    )
    ...
    validation_audit, _ = pair_audit.evaluate_split(
        ...
        args.mc_samples, args.lab_batch_size, selected_seed + 910_000, threshold,
    )
    test_audit, _ = pair_audit.evaluate_split(
        module, model, lab_scaler, lab_data["test"], permutations, edge_permutations,
        args.mc_samples, args.lab_batch_size, selected_seed + 920_000, threshold,
    )
