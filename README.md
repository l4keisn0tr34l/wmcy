...
**Current status:** equal-duration V2 is complete and removes the original late-negative censoring shortcut. An independent compact RSSM now performs six-step stochastic prior rollout: test state MAE is 0.280 versus 0.354 Ridge and 0.384 persistence; future-LM F1 is 0.800 versus 0.645 Ridge. CUDA training is verified on the local RTX 3050 (`--device auto|cpu|cuda`); use prespecified larger batches because batch 32 is launch-overhead bound. A permutation-equivariant graph RSSM reaches V2 state MAE 0.252 and edge AP 0.394. Observable-only UNSW pretraining helps some V2 metrics, but fresh matched-prefix V3 selects scratch initialization: state MAE 0.294, edge AP 0.808, 3/3 progressing episodes detected 23.5 seconds early—and 3/3 same-prefix stopped episodes alert. This reveals a passive-observability boundary for unobserved future attacker action. A validation-only outcome-conditioned two-branch graph RSSM now represents no-LM/LM alternatives explicitly (expected/oracle state MAE 0.327/0.306; exact-LM Brier 0.114), but still alerts on all stopped/progressing pairs. The 36-episode V4 action/passive corpus is captured and frozen. On five sealed permit/block pairs, action-conditioned scratch forecasting reaches state MAE 0.060, edge/LM/pair AP 1.0, and gives lower factual than opposite-action state error on 10/10 contexts; fixed Friday initialization is worse at state MAE 0.072. The frozen passive branch model transfers poorly as an early-warning gate: it alerts 3/3 stopped prefixes but 0/3 progressing prefixes before a positive horizon and misses 3/3 direct-credential progressions. The full 8.839 GB Friday PCAPNG is also canonicalized into 2,102,560 label-free directed events and 5,775 train-only graph sequences. A new five-host V5 corpus now has 80 validated equal-duration episodes: every capture is ~150s with29 complete dense states and zero packet drops; model training and test evaluation have not started. See `docs/V5_CORPUS_AUDIT.md`, `docs/V4_ACTION_RESULTS.md`, and linked result documents; findings remain small controlled-lab evidence, not enterprise generalization or calibrated uncertainty.
