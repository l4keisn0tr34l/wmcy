...
**Current status:** V5 completed one reviewed, frozen, sealed evaluation on 80 equal-duration five-host episodes. Given a preselected permit/block action and pair, scratch state MAE is 0.460 versus 0.540 persistence, edge AP is 0.411, and the factual action gives lower state error in 8/8 contexts. Without action input, passive two-future dynamics reach state MAE 0.473 versus 0.539 persistence, but completed-LM F1 at the primary 0.5 threshold is 0 and oracle branch gain is only 0.002. The frozen low threshold detects 3/6 progressing episodes while falsely alerting 7/10 nonprogressing episodes. These are controlled-lab dynamics results—not enterprise generalization, calibrated uncertainty, hidden-intent recovery, or causal-policy evidence. Test inference is permanently closed; reporting uses saved artifacts only. See `docs/V5_TEST_RESULTS.md`, the standalone report, and judge deck listed in `docs/OUTPUT_LOCATIONS.md`.
